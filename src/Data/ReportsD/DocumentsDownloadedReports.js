...
const DocumentsDownloadedReports = () => {
    ...
    // Build parameters for downloaded-files report endpoint
    const buildDownloadedFilesParams = (flagType) => ({
        branchId:
            currRole === SYSTEM_ADMIN
    ...
    const handleDownloadReport = async (flagType) => {
        ...
        try {
            const response = await apiClient.get(
                `${API_HOST}/jasper-report/downloaded-files`,
                {
                    params: buildDownloadedFilesParams(flagType),
                    responseType: "blob",
                }
            );
    ...
    const handleExportExcel = async () => {
        ...
        try {
            const response = await apiClient.get(
                `${API_HOST}/jasper-report/downloaded-files`,
                {
                    params: buildDownloadedFilesParams("E"),
                    responseType: "blob",
                }
            );
            ...
            link.href = url;
            link.setAttribute("download", "Downloaded_Files_Report.xlsx");
    ...
    return (
        <div className="p-4-">
            <div className="title">
                <h1><AutoTranslate>Document Downloaded Reports</AutoTranslate></h1>
            </div>
